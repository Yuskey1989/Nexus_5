/*
 * drivers/cpufreq/cpufreq_adaptactive.c
 *
 ...
 * Author: Pranav Vashi (neobuddy89@gmail.com) Modified for impulse
 * Author: Yusuke Fukutsuka (donsuke.f@gmail.com) Modified for adaptactive
 */
...
static void cpufreq_interactive_timer(unsigned long data)
{
	...
	} else if (cpu_load <= go_lowspeed_load && !boost_val) {
		boosted = false;
...
static void cpufreq_interactive_boost(void)
{
	...
	for_each_online_cpu(i) {
		...
		if (now < input_boostpulse_endtime) {
...
static int cpufreq_governor_adaptactive(struct cpufreq_policy *policy,
		unsigned int event)
{
	int rc;
	unsigned int j;
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_ADAPTACTIVE
static
#endif
struct cpufreq_governor cpufreq_gov_adaptactive = {
	.name = "adaptactive",
	.governor = cpufreq_governor_adaptactive,
...
static int __init cpufreq_adaptactive_init(void)
{
	unsigned int i, rc;
	struct cpufreq_interactive_cpuinfo *pcpu;
	...
	return cpufreq_register_governor(&cpufreq_gov_adaptactive);
}

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_ADAPTACTIVE
fs_initcall(cpufreq_adaptactive_init);
#else
module_init(cpufreq_adaptactive_init);
#endif
...
static void __exit cpufreq_interactive_exit(void)
{
	...
	cpufreq_unregister_governor(&cpufreq_gov_adaptactive);
...
MODULE_DESCRIPTION("'cpufreq_adaptactive' - A cpufreq governor for "
